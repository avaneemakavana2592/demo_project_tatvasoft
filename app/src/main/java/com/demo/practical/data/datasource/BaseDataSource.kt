...
import com.demo.practical.exception.ServerException
                ...
                .map {
                    if (it.responseBody != null) {
                        when (it.responseBody.status) {
                            /*true*/0, 2, 3, 4, 11 -> return@map DataWrapper(it.responseBody, ServerException(it.responseBody.message, it.responseBody.status))
                        }
                    }
                    return@map it
                }
